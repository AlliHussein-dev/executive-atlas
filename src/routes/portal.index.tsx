...
function PortalOverview() {
  return (
    <PortalShell>
      ...
      <div className="grid lg:grid-cols-3 gap-5 sm:gap-6 lg:gap-8 mb-10 sm:mb-12">
        {/* Advisor card */}
        <div className="lg:col-span-2 bg-white border border-navy/10 p-6 sm:p-8 lg:p-10">
          ...
          <div className="flex flex-col sm:flex-row items-start gap-5 sm:gap-6">
            <img
              src={advisor.image}
              alt={advisor.name}
              className="w-20 h-20 sm:w-24 sm:h-24 lg:w-32 lg:h-32 object-cover grayscale rounded-sm flex-shrink-0"
            />
            <div className="min-w-0 w-full">
              <h2 className="font-display text-xl sm:text-2xl lg:text-3xl text-navy">
                {advisor.name}
              </h2>
              <p className="text-[11px] uppercase tracking-[0.2em] text-slate font-semibold mt-2">
                {advisor.title}
              </p>
              <div className="mt-4 sm:mt-5 space-y-1.5 text-sm text-slate">
                <p>{advisor.phone}</p>
                <p className="break-all">{advisor.email}</p>
              </div>
        ...
        <div className="bg-navy text-sand p-6 sm:p-8 lg:p-10">
          <p className="text-[10px] uppercase tracking-[0.25em] text-gold font-semibold mb-6">
            Upcoming Meetings
          </p>
          <ul className="space-y-5 sm:space-y-6">
            {upcomingMeetings.map((m) => (
              <li
                key={m.id}
                className="border-b border-sand/10 last:border-0 pb-5 last:pb-0"
              >
      ...
      {/* Deadlines */}
      <div className="bg-white border border-navy/10 p-6 sm:p-8 lg:p-10 mb-10 sm:mb-12">
        <div className="flex items-end justify-between mb-6 sm:mb-8 gap-4">
          <div>
            <p className="text-[10px] uppercase tracking-[0.25em] text-gold font-semibold mb-2">
              Important Deadlines
            </p>
            <h2 className="font-display text-xl sm:text-2xl text-navy">Coming up</h2>
          </div>
          <Link
            to="/portal/progress"
            className="text-[10px] uppercase tracking-[0.2em] font-bold border-b border-gold pb-1 text-navy whitespace-nowrap"
          >
            View Progress
          </Link>
        </div>
        <ul className="divide-y divide-navy/10">
          {deadlines.map((d) => (
            <li key={d.label} className="flex items-center gap-4 sm:gap-6 py-4">
              <span className="font-display text-xl sm:text-2xl text-navy w-14 sm:w-16 flex-shrink-0">
                {d.date}
              </span>
      ...
      <div className="grid sm:grid-cols-3 gap-4 sm:gap-5">
        {[
          ...
          <Link
            key={q.to}
            to={q.to}
            className="group bg-ivory border border-navy/10 p-6 sm:p-8 hover:border-gold transition-colors"
          >
            <p className="text-gold font-display italic text-xl mb-3 sm:mb-4">{q.n}</p>
            <p className="font-display text-lg sm:text-xl text-navy mb-4">{q.label}</p>
